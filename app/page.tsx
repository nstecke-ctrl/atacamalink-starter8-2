...
function LocalProductCard({ p }: { p: any }) {
  const { addItem } = useCart();
  const slug = p.slug || slugify(`${p.brand}-${p.model}`);
  const id = p.id || slug;
  const isHanwha = String(p.brand || '').toLowerCase().includes('hanwha');
  const priceStr = formatMoney(p.price, p.currency);
  function handleAdd() {
    addItem({ id, brand: p.brand, model: p.model, price: p.price, slug, category: p.category, image: p.image, blurb: p.blurb }, 1);
  }
  return (
    <div className="rounded-2xl border overflow-hidden hover:shadow-sm transition-all bg-white">
      <div className="relative" style={{ height: 208, padding: 12, overflow: 'hidden', background: '#fff', display: 'grid', placeItems: 'center', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
        ...
        <img src={p.image || '/brands/hanwha-placeholder.png'} alt={`${p.brand} ${p.model}`} style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }}
             onError={(e) => { (e.currentTarget as HTMLImageElement).src = '/brands/hanwha-placeholder.png'; }}/>
      </div>
      <div className="p-4">
        <div className="text-xs opacity-60">{p.category}</div>
        <h3 className="mt-1 text-lg font-semibold">{p.brand} {p.model}</h3>
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          ...
          <Link href={`/producto/${slug}`} className="text-sm underline underline-offset-4 hover:no-underline text-teal-700">Ver detalle</Link>
          {!isHanwha && p.datasheet && (
            <a href={p.datasheet} target="_blank" rel="noopener noreferrer" className="text-sm underline underline-offset-4 hover:no-underline text-teal-700">Ficha técnica</a>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50">
        ...
        <div className="grid gap-6 lg:grid-cols-2 items-center">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mt-4">
              Tecnología confiable para entornos mineros exigentes
            </h1>
            <p className="mt-3 text-gray-700">
              Radios, cámaras, antenas, control de acceso y networking industrial. Integración, asesoría y soporte local.
            </p>
            <div className="mt-6 flex gap-3 flex-wrap">
              <Link href="/productos/camaras" className="rounded-2xl border px-4 py-2 text-sm">Ver cámaras</Link>
              <Link href="/productos/radios" className="rounded-2xl border px-4 py-2 text-sm">Ver radios</Link>
              <Link href="/productos" className="rounded-2xl px-4 py-2 text-sm text-white" style={{ backgroundColor: ACCENT }}>Ver catálogo</Link>
            </div>
          </div>
      ...
      <Section id="destacados" className="py-12">
        <div className="flex items-end justify-between mb-2">
          ...
          <Link href="/productos" className="text-sm underline underline-offset-4 hover:no-underline" style={{ color: '#E76F51' }}>
            Ver catálogo
          </Link>
        </div>
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {featured.map((p) => (<LocalProductCard key={`${p.brand}-${p.model}`} p={p} />))}
        </div>
      </Section>
    </div>
  );
