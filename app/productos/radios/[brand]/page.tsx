...
export default function Page() {
  ...
  const list = useMemo(() => {
    return items.filter((p) => isRadio(p) && norm(p.brand).includes(brandParam));
  }, [items, brandParam]);

  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-bold mb-4">Radios — {params?.brand}</h1>
      ...
      <div className="mt-8">
        <Link href="/productos/radios" className="inline-block rounded-2xl px-4 py-2 text-white" style={{ backgroundColor: ACCENT }}>
          Volver a radios
        </Link>
      </div>
    </div>
